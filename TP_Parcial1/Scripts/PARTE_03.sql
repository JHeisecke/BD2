...
SELECT S.NOMBRE||' '||S.APELLIDO as "Nombre y Apellido", SUM(DECODE(AC.INGRESO_GASTO, 'I',AD.IMPORTE,0)-DECODE(AC.INGRESO_GASTO,'G',AD.IMPORTE,0)) Ingresos,
SUM(AI.VALOR_FISCAL) as "Valor Activos", CP.NRO_PRESTAMO as "Nro Prestamo", sum(NVL(CA.SALDO_DISPONIBLE,0)) Disponible, 
NVL(CP.MONTO_PRESTAMO,0) as "Monto Prestamo", CP.FECHA_DESEMBOLSO as "Fecha Desembolso", MAX(CC.FECHA_PAGO) as "Ultima fecha de pago", 
SUM((CASE WHEN CC.FECHA_PAGO IS NOT NULL THEN CC.MONTO_CUOTA ELSE 0 END) - CC.INTERES_MORATORIO) AS "Monto cuotas pagadas",
SUM((CASE WHEN CC.FECHA_PAGO IS NULL THEN CC.MONTO_CUOTA ELSE 0 END)) AS "Monto cuotas pendientes"
FROM SOC_SOCIO S
